...
function migrate(raw:unknown):Phase3CState{if(!object(raw)||raw.version!==1)return blank();const checklist:Array<PersonalChecklistItem>=[];if(Array.isArray(raw.checklist))for(const x of raw.checklist){if(!object(x)||typeof x.id!=='string'||typeof x.done!=='boolean'||typeof x.createdAt!=='string')continue;const label=cleanString(x.label,240).trim();if(label)checklist.push({id:x.id.slice(0,120),label,done:x.done,createdAt:x.createdAt})}return {version:1,hajj:cleanTrack(raw.hajj,'hajj'),umrah:cleanTrack(raw.umrah,'umrah'),checklist:checklist.slice(-200)}}
export const loadPhase3CState=():Phase3CState=>{try{const raw=localStorage.getItem(KEY);return raw?migrate(JSON.parse(raw)):blank()}catch{return blank()}}
