...
describe('zakat calculation',()=>{
 it('values all declared asset classes and applies explicit deductions',()=>{const r=calculateZakat({...base,eligibleDeductions:25000});expect(r.grossAssets).toBe(427000);expect(r.zakatableTotal).toBe(402000);expect(r.estimatedZakat).toBe(10050)})
 ...
 it('does not deduct nisab from the zakatable total',()=>{const r=calculateZakat({...base,cash:100000,goldGrams:0,silverGrams:0,eligibleDeductions:0,nisabMethod:'gold',goldPricePerGram:7000});expect(r.zakatableTotal).toBe(350000);expect(r.estimatedZakat).toBe(0)})
