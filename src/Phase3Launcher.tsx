...
import { loadState, localDateKey, saveState, type AppState, type SalahName } from './storage'
...
export default function Phase3Launcher() {
  ...
  const [calendarAdjustment, setCalendarAdjustment] = useState(0)

  useEffect(() => { if (!open) return; setState(loadState()); setDraft(readLastLocation()); setNow(new Date()); void Promise.all([loadQuran(), loadVerifiedContent(), loadQuranTranslations()]).then(([q, content, translations]) => { setQuran(q); setDailyContent({ names: content.names, duas: content.duas, azkar: content.azkar, translations: translations.translations }) }).catch(e => setError(e instanceof Error ? e.message : 'Verified content could not be loaded.')) }, [open])
  useEffect(() => { if (!open) return; saveState(state); setDraft(s => ({ ...s, label: state.location?.label ?? s.label, lat: state.location ? String(state.location.lat) : s.lat, lon: state.location ? String(state.location.lon) : s.lon })) }, [state, open])
  ...
  const quranRead = Object.keys(state.version === 3 ? loadState().version === 3 ? {} : {} : {}).length
  ...
  const monthDays = calendar.days

  const setLocation = (lat: number, lon: number, label: string) => {
    try {
      const tz = timezoneFromCoordinates(lat, lon)
      const nextState: AppState = { ...state, location: { lat, lon, label: label.trim().slice(0, 120) || 'Selected location', timeZone: tz } }
      setState(nextState)
      localStorage.setItem(LOCATION_KEY, JSON.stringify({ label: nextState.location?.label, lat, lon }))
      setMessage(`Prayer calculation location set to ${nextState.location?.label}.`)
      setError('')
