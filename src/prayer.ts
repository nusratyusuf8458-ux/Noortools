...
export type PrayerSettings = {
  ...
  asrMethod?: AsrMethod
  hanafi?: boolean
  highLatitude?: HighLatitudeMethod
  adjustments?: PrayerAdjustments
}
export type Prayer = { name: PrayerName; time: Date }

export type PrayerCalculationMetadata = {
  method: CalculationMethod
  asrMethod: AsrMethod
  highLatitude: HighLatitudeMethod
  adjustments: PrayerAdjustments
...
function solarPosition(date: CalendarDate, lon: number): SolarPosition {
  ...
  return { declination, equationOfTime, solarNoonUtcMinutes: 720 - 4 * lon - equationOfTime }
}
...
function solarTime(date: CalendarDate, lat: number, lon: number, altitudeDegrees: number, morning: boolean) {
  ...
  const cosHourAngle = (Math.sin(altitude) - Math.sin(rad(lat)) * Math.sin(declination)) / denominator
...
function toLocalInstant(date: CalendarDate, timeZone: string, localHourValue: number) {
  ...
  let guess = base
  for (let i = 0; i < 6; i += 1) guess = base - timezoneOffsetMinutes(new Date(guess), timeZone) * 60000
  return new Date(guess)
}

function fallbackFraction(method: HighLatitudeMethod, angle: number) {
  if (method === 'middleOfNight') return 0.5
  if (method === 'oneSeventh') return 1 / 7
  return angle / 60
}

function fallbackFajr(sunrise: Date, nightHours: number, method: HighLatitudeMethod, angle: number) {
  return new Date(sunrise.getTime() - nightHours * fallbackFraction(method, angle) * 3600000)
}
...
function fallbackIsha(sunset: Date, nextSunrise: Date, method: HighLatitudeMethod, angle: number) {
  ...
  return new Date(sunset.getTime() + nightHours * fallbackFraction(method, angle) * 3600000)
}

function instantAtLocalNoon(date: CalendarDate, timeZone: string) {
  return toLocalInstant(date, timeZone, 12)
}

function adjustment(settings: PrayerSettings, name: 'Fajr' | 'Dhuhr' | 'Asr' | 'Maghrib' | 'Isha') {
  const value = settings.adjustments?.[name]
  return typeof value === 'number' && Number.isFinite(value) ? value : 0
}

function adjusted(time: Date | null, settings: PrayerSettings, name: 'Fajr' | 'Dhuhr' | 'Asr' | 'Maghrib' | 'Isha') {
  if (!(time instanceof Date) || !Number.isFinite(time.getTime())) return null
  return new Date(time.getTime() + adjustment(settings, name) * 60000)
}
...
export function calculatePrayerTimes(date: Date, lat: number, lon: number, timeZone = 'UTC', settings: PrayerSettings = {}): Prayer[] {
  ...
  if (highLatitude !== 'none' && sunrise && astronomicalSunset && nextSunrise) {
    const nightHours = (nextSunrise.getTime() - astronomicalSunset.getTime()) / 3600000
    if (!resolvedFajr) resolvedFajr = fallbackFajr(sunrise, nightHours, highLatitude, config.fajrAngle)
    if (!resolvedIsha) resolvedIsha = fallbackIsha(astronomicalSunset, nextSunrise, highLatitude, config.ishaAngle ?? 18)
  }

  const events: Array<[PrayerName, Date | null]> = [
    ['Fajr', adjusted(resolvedFajr, settings, 'Fajr')],
    ['Sunrise', sunrise],
    ['Dhuhr', adjusted(dhuhr, settings, 'Dhuhr')],
    ['Asr', adjusted(asrTime, settings, 'Asr')],
    ['Maghrib', adjusted(maghrib, settings, 'Maghrib')],
    ['Isha', adjusted(resolvedIsha, settings, 'Isha')],
  ]
  void previousSunset
...
export function nextPrayer(prayers: Prayer[], lat: number, lon: number, timeZone: string, settings: PrayerSettings = {}, now = new Date()) {
  const upcoming = prayers.filter(p => p.name !== 'Sunrise').find(p => p.time.getTime() > now.getTime())
...
export function currentPrayer(prayers: Prayer[], now = new Date()): Exclude<PrayerName, 'Sunrise'> | null {
  let current: Exclude<PrayerName, 'Sunrise'> | null = null
...
export function prayerTimeLabel(date: Date, timeZone: string, use24Hour = false) {
  return new Intl.DateTimeFormat('en-US', { timeZone, hour: 'numeric', minute: '2-digit', hour12: !use24Hour }).format(date)
}

export function nightMidpoint(date: Date, lat: number, lon: number, timeZone: string, settings: PrayerSettings = {}) {
  const prayers = calculatePrayerTimes(date, lat, lon, timeZone, settings)
  const sunset = prayers.find(p => p.name === 'Maghrib')
  const tomorrow = addDays(calendarDate(date, timeZone), 1)
  const tomorrowPrayers = calculatePrayerTimes(instantAtLocalNoon(tomorrow, timeZone), lat, lon, timeZone, settings)
  const tomorrowFajr = tomorrowPrayers.find(p => p.name === 'Fajr')
  if (!sunset || !tomorrowFajr) return null
  return new Date(sunset.time.getTime() + (tomorrowFajr.time.getTime() - sunset.time.getTime()) / 2)
}
