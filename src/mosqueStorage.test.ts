...
describe('Phase 3D mosque cache', () => {
  ...
  it('sorts cached results by the chosen origin without changing provider data', () => { const second = { ...item, id: 'osm:node:2', name: 'Near', coordinates: { lat: 19.001, lon: 72.001 } }; saveMosqueCache({ lat: 19, lon: 72 }, 'x', [item, second], 1000); expect(cachedResultsForDisplay(loadMosqueCache(2000), { lat: 19.002, lon: 72.002 })[0].name).toBe('Near') })
