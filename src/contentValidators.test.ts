...
const unavailableSource = { source: 'Not connected', version: '0', license: 'not applicable', sourceUrl: 'https://example.invalid', verificationStatus: 'unavailable' as const }
...
describe('content validators', () => {
  ...
  it('requires 99 source-defined records before a Names dataset can be verified', () => {
    const items = Array.from({ length: 98 }, (_, index) => ({ id: `name:${index + 1}`, type: 'allah_name', arabic: 'source item', source: { ...unavailableSource, verificationStatus: 'needs_review' as const } })) as ReligiousContentItem[]
    expect(validateNamesDataset(items).status).toBe('needs_review')
  })

  it('requires an explicit Dua reference and preserves source metadata rules', () => {
    const dua = { id: 'dua:1', type: 'dua', arabic: 'source text', source: { ...unavailableSource }, count: 1 } as ReligiousContentItem & { count: number }
  ...
  it('never accepts an Azkar count of zero or a fractional value', () => {
    const azkar = { id: 'azkar:1', type: 'azkar', arabic: 'source text', source: { ...unavailableSource, reference: 'source reference' }, count: 1 } as ReligiousContentItem & { count: number }
  ...
  it('requires a real hadith collection and reference', () => {
    const hadith = { id: 'hadith:1', type: 'hadith', arabic: 'source text', source: { ...unavailableSource }, reference: 'Collection 1:1', collection: 'Collection' } as ReligiousContentItem & { reference: string; collection: string }
