...
Caching: fetched mosque results are stored only in bounded local storage for up to 24 hours as a convenience cache. Cached results are visibly marked as cached and may be stale; they are never presented as live. No background refresh is performed. A user-selected precise location is not persisted by this feature unless the user explicitly confirms Save; the feature's default live location remains in memory for the current session.
...
## Available fields
The OSM adapter may expose: name, structured address components when mapped, coordinates, phone/contact phone, website/contact website, opening_hours, selected source-tagged facilities, source record URL, and provider timestamp when supplied. Missing fields render as unavailable. A structured open/closed status is not derived from raw opening-hours text.
...
Previously cached results can be displayed after a failed live request with a cache-age warning. The cache is removed after 24 hours. No offline OSM tile archive is created.
...
## Testing
Coverage includes coordinate validation, Haversine straight-line distance, distance sorting, duplicate elimination, provider-field mapping, malformed geocoder responses, cache behavior, cached-result display, report packaging, permission-denial fallback messaging, timezone resolution, and the critical rule that missing mosque timetables stay unavailable rather than falling back to NoorTools calculated prayer times. Mosque-specific past/upcoming logic is tested only for a supplied timetable and explicit timezone.
